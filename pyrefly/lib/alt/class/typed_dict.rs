...
/// Why a single TypedDict field did or did not materialize. The failure modes differ only in
/// whether `typed_dict_fields` may cache the map it built.
enum FieldOutcome {
    Field(TypedDictField),
    /// The member cannot be a field (property, descriptor, no annotation).
    PermanentlyAbsent,
    /// Failed for a reason a later call may not hit, such as a stale class mid-update. No known
    /// input reaches this, but assuming permanence would cache a map missing a real field.
    TransientFailure,
}
